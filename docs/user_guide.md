...
- **Sonde associée** (1.47.0) : la sonde qui confirme un appairage devient LA sonde de ce hub (mémorisée en NVS). Ensuite, seules ses trames sont archivées ; une autre sonde à portée (établi, tests) est ignorée, avec un log au plus toutes les 10 min (`trame ignoree : sonde … non associee`). Tant qu'aucune sonde n'a confirmé d'appairage, toutes sont acceptées. `GET /api/live` indique la sonde associée (`out.sensor_mac`). Le suivi de synchronisation est tenu **par MAC de sonde** : deux sondes partageant le même `node_id` ne se mélangent plus. Les logs `[OUT]` affichent la fin de la MAC et la durée d'éveil de la sonde au moment de l'envoi (`up=…s`).
